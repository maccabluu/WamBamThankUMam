#!/usr/bin/env python3
from pathlib import Path
import re
import sys


def require_replace(text, old, new, label, minimum=1):
    count = text.count(old)
    if count < minimum:
        raise SystemExit(f"Missing patch point {label}: found {count}, need {minimum}")
    return text.replace(old, new)


def patch_overlay(text: str) -> str:
    marker = 'const ID = "wambam-alpha02-overlay";'
    start = text.find(marker)
    if start < 0:
        raise SystemExit("Wam Bam game overlay marker not found")
    end_marker = 'openIncomingCurtain();\n})();'
    end = text.find(end_marker, start)
    if end < 0:
        raise SystemExit("Wam Bam game overlay end marker not found")
    end += len(end_marker)
    before, seg, after = text[:start], text[start:end], text[end:]

    seg = require_replace(
        seg,
        'const levelConfig = level => LEVEL_CONFIGS[level] || LEVEL_CONFIGS[1];',
        'const levelConfig = level => LEVEL_CONFIGS[level] || LEVEL_CONFIGS[1];\n  const boardRowsForLevel = level => level === 1 ? 11 : 8;',
        'row helper'
    )

    seg = require_replace(
        seg,
        '''    st.coinsEl = label(76.8,2.65,14.4,3.05);\n    st.coinsEl.style.fontSize = "clamp(13px,2.05vw,22px)";\n    st.livesEl = label(87.3,9.05,8.0,3.7);\n    st.livesEl.style.fontSize = "clamp(17px,2.55vw,27px)";\n    st.livesEl.style.lineHeight = "1";''',
        '''    st.coinsEl = st.level === 1 ? null : label(76.8,2.65,14.4,3.05);\n    if (st.coinsEl) st.coinsEl.style.fontSize = "clamp(13px,2.05vw,22px)";\n    st.livesEl = st.level === 1 ? null : label(87.3,9.05,8.0,3.7);\n    if (st.livesEl) st.livesEl.style.fontSize = "clamp(17px,2.55vw,27px)";\n    if (st.livesEl) st.livesEl.style.lineHeight = "1";''',
        'remove Level 1 coin and heart counters'
    )

    seg = require_replace(
        seg,
        '    const targetRows = [25.1,29.0,32.9];',
        '    const targetRows = st.level === 1 ? [8.4,12.5,16.6] : [25.1,29.0,32.9];',
        'Level 1 target rows'
    )
    seg = require_replace(seg, '      if (st.level === 3 || st.level === 5) {', '      if (st.level === 1 || st.level === 3 || st.level === 5) {', 'Level 1 target icons')
    seg = require_replace(seg, '          left: "5.8%",', '          left: st.level === 1 ? "78.5%" : "5.8%",', 'target icon left')
    seg = require_replace(seg, '          top: `${targetRows[index] - .8}%`,', '          top: `${targetRows[index] - (st.level === 1 ? .7 : .8)}%`,', 'target icon top')
    seg = require_replace(seg, '          width: "7%",', '          width: st.level === 1 ? "7.8%" : "7%",', 'target icon width')
    seg = require_replace(seg, '          height: "3.6%",', '          height: st.level === 1 ? "3.1%" : "3.6%",', 'target icon height')
    seg = require_replace(
        seg,
        '      st.targetEls[key] = label(13.0,targetRows[index] - .25,7.2,2.5,"#171717");',
        '      st.targetEls[key] = st.level === 1\n        ? label(87.1,targetRows[index] - .25,7.0,2.5,"#171717")\n        : label(13.0,targetRows[index] - .25,7.2,2.5,"#171717");',
        'target number placement'
    )

    seg = require_replace(
        seg,
        '      ? {left:14.1, top:38.0, width:74.0, height:45.7}',
        '      ? {left:14.1, top:21.2, width:74.0, height:62.3}',
        'new Level 1 board bounds'
    )
    seg = require_replace(
        seg,
        '    board.style.gridTemplateRows = "repeat(8,1fr)";',
        '    board.style.gridTemplateRows = `repeat(${boardRowsForLevel(st.level)},1fr)`;',
        '11 row grid'
    )
    seg = require_replace(
        seg,
        '    board.style.transition = "all 180ms ease";',
        '    board.style.transition = st.level === 1 ? "none" : "all 180ms ease";',
        'board transition optimization'
    )

    # All column counts stay at 8 to match the visible background. Level 1 grows vertically to 11 cells.
    # Replace row-specific limits only.
    seg = re.sub(r'\br\s*<\s*8\b', 'r < boardRowsForLevel(st.level)', seg)
    seg = re.sub(r'\brr\s*<\s*8\b', 'rr < boardRowsForLevel(st.level)', seg)
    seg = re.sub(r'\br\s*<\s*7\b', 'r < boardRowsForLevel(st.level) - 1', seg)

    seg = require_replace(
        seg,
        'while (end < 8 && matchValue(end,c) === value) end++;',
        'while (end < boardRowsForLevel(st.level) && matchValue(end,c) === value) end++;',
        'vertical match end',
        minimum=2
    )
    seg = require_replace(
        seg,
        'st.board = Array.from({length:8}, () => Array(8).fill(0));',
        'st.board = Array.from({length:boardRowsForLevel(st.level)}, () => Array(8).fill(0));',
        'board creation'
    )
    seg = require_replace(seg, 'if (r < 7 && swapCreatesMatch(r,c,r+1,c)) return true;', 'if (r < boardRowsForLevel(st.level) - 1 && swapCreatesMatch(r,c,r+1,c)) return true;', 'vertical move check')
    seg = require_replace(seg, 'for (let r = 7; r >= 1; r--) {', 'for (let r = boardRowsForLevel(st.level) - 1; r >= 1; r--) {', 'settle bottom row')
    seg = require_replace(seg, '        let write=7;', '        let write=boardRowsForLevel(st.level) - 1;', 'collapse write row')
    seg = require_replace(seg, '        for(let r=7;r>=0;r--){', '        for(let r=boardRowsForLevel(st.level)-1;r>=0;r--){', 'collapse row loop')

    # Rocket clears one full row and one full column. Rows and columns now have different counts on Level 1.
    seg = require_replace(
        seg,
        '''        damageHandbags([\n          ...Array.from({length:8},(_,i)=>handbagKey(r,i)),\n          ...Array.from({length:8},(_,i)=>handbagKey(i,c))\n        ]);\n        for (let i=0;i<8;i++) {\n          st.board[r][i] = null;\n          st.board[i][c] = null;\n        }''',
        '''        damageHandbags([\n          ...Array.from({length:8},(_,i)=>handbagKey(r,i)),\n          ...Array.from({length:boardRowsForLevel(st.level)},(_,i)=>handbagKey(i,c))\n        ]);\n        for (let i=0;i<8;i++) st.board[r][i] = null;\n        for (let i=0;i<boardRowsForLevel(st.level);i++) st.board[i][c] = null;''',
        'rocket row and column sizes'
    )

    # One-pass Level 1 collapse avoids rebuilding 88 buttons once per falling cell.
    fast_settle_anchor = '''      const cols = columns && columns.length\n        ? [...new Set(columns)]\n        : [0,1,2,3,4,5,6,7];\n\n      // Candy-Crush-style loop:'''
    fast_settle_replacement = '''      const cols = columns && columns.length\n        ? [...new Set(columns)]\n        : [0,1,2,3,4,5,6,7];\n\n      if (st.level === 1) {\n        st.dropMoves = collapse(cols);\n        st.dropColumns = cols;\n        st.dropAnimating = true;\n        render();\n        await new Promise(res => setTimeout(res, 125));\n        st.dropAnimating = false;\n        st.dropMoves = [];\n        st.dropColumns = [];\n        render();\n        return;\n      }\n\n      // Candy-Crush-style loop:'''
    seg = require_replace(seg, fast_settle_anchor, fast_settle_replacement, 'fast Level 1 collapse')

    seg = require_replace(
        seg,
        '      st.coinsEl.textContent=st.coins.toLocaleString();\n      st.livesEl.textContent=st.lives;',
        '      if (st.coinsEl) st.coinsEl.textContent=st.coins.toLocaleString();\n      if (st.livesEl) st.livesEl.textContent=st.lives;',
        'HUD render guards'
    )

    seg = require_replace(
        seg,
        '      board.innerHTML="";',
        '      board.innerHTML="";\n      const boardFragment = document.createDocumentFragment();',
        'board fragment start'
    )
    seg = require_replace(
        seg,
        '''          b.style.backgroundSize = bagState\n            ? (bagState === 1 ? "86% auto" : "82% auto")\n            : iconFit[pieceKey] || "108% 108%";''',
        '''          b.style.backgroundSize = bagState\n            ? (bagState === 1 ? "86% auto" : "82% auto")\n            : st.level === 1\n              ? "82% auto"\n              : iconFit[pieceKey] || "108% 108%";''',
        'Level 1 icon sizing'
    )
    seg = require_replace(
        seg,
        '          b.style.transform=bagState ? "scale(.9)" : "scale(.78)";',
        '          b.style.transform=bagState ? "scale(.9)" : st.level === 1 ? "scale(.94)" : "scale(.78)";',
        'Level 1 tile fit'
    )
    seg = require_replace(
        seg,
        '          b.style.transition="transform 180ms ease, opacity 180ms ease";',
        '          b.style.transition=st.level === 1 ? "transform 120ms ease, opacity 120ms ease" : "transform 180ms ease, opacity 180ms ease";',
        'Level 1 tile transition'
    )
    seg = require_replace(
        seg,
        '''          board.appendChild(b);\n        }\n      }\n    }\n\n    makeBoard();''',
        '''          boardFragment.appendChild(b);\n        }\n      }\n      board.appendChild(boardFragment);\n    }\n\n    makeBoard();''',
        'board fragment finish'
    )

    if 'repeat(${boardRowsForLevel(st.level)},1fr)' not in seg:
        raise SystemExit('11-row CSS grid patch missing')
    if '? {left:14.1, top:21.2, width:74.0, height:62.3}' not in seg:
        raise SystemExit('new background board alignment missing')

    return before + seg + after


def main():
    if len(sys.argv) != 2:
        raise SystemExit('Usage: patch_level1_real_apk.py <game-overlay.js-or-code2.js>')
    path = Path(sys.argv[1])
    source = path.read_text(encoding='utf-8')
    patched = patch_overlay(source)
    path.write_text(patched, encoding='utf-8')
    print(f'Patched real Level 1 gameplay in {path}')


if __name__ == '__main__':
    main()
