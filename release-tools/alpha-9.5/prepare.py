#!/usr/bin/env python3
"""Apply the 9.5 web game to the verified 9.3 APK's extracted www folder.
...
data.write_text(project.replace('"version":"9.3.0"', '"version":"9.5.0"', 1))
...
text, count = re.subn(r'const CURRENT_VERSION = "[^"]+";', 'const CURRENT_VERSION = "9.5.0";', text, count=1)
...
print(json.dumps({'version': '9.5.0', 'levels': 40, 'new_icons': len(list((root/'icons').glob('*.png'))), 'www': str(root)}))
