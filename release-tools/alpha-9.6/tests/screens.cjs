...
async function main(){
  ...
  localStorage.setItem('wambam-unlocked-level','40');localStorage.setItem('wambam-selected-level','6');
  ...
  const server=http.createServer(async(req,res)=>{try{
    ...
    const mime={'.js':'text/javascript; charset=utf-8','.html':'text/html; charset=utf-8','.png':'image/png','.jpg':'image/jpeg','.mp3':'audio/mpeg','.svg':'image/svg+xml'}[path.extname(file)];
  ...
  try{
    ...
    assert.ok(await page.locator('.wam-node-stars').first().textContent()==='✓');
    ...
    assert.ok(await page.locator('.wam-map-world').evaluate(e=>getComputedStyle(e).backgroundImage.includes('artwork/road-map.jpg')));
    await page.evaluate(async()=>{const img=new Image();img.src='artwork/road-map.jpg';await img.decode();});
    await page.screenshot({path:path.join(screenshots,'map-phone.png')});
    for(const level of [1,5,6,8,9,10,14,20,21,30,31,40]){
      ...
      assert.equal(await page.locator('.wam-grid.shaped').count(),1);
      assert.equal(await page.locator('.wam-art').getAttribute('src'),level>=21&&level<=30?'artwork/disco-diner.jpg':'game_background_90.png');
      ...
      assert.equal(await page.evaluate(()=>qa.scene.__wam94Game.game.board.filter(Boolean).length+qa.scene.__wam94Game.game.bags.size+qa.scene.__wam94Game.game.crates.size+qa.scene.__wam94Game.game.holes.size),await page.evaluate(()=>qa.scene.__wam94Game.game.rows*qa.scene.__wam94Game.game.cols));
    ...
    await page.screenshot({path:path.join(screenshots,'area-decorations.png')});
    ...
    await page.evaluate(()=>{const v=qa.scene.__wam94Game,g=v.game;g.board.forEach((p,i)=>{if(p){p.type=g.config.palette[(Math.floor(i/g.cols)*2+i%g.cols)%5];p.special=null;}});g.targets={lips:1,bow:0,heel:0};g.moves=1;g.board[9]=g.piece('disco','disco');g.board[10]=g.piece('lips');v.render();});
    await page.locator('[data-cell="9"]').click();await page.locator('[data-cell="10"]').click();
    ...
    await page.locator('.wam-grid').waitFor();assert.equal(await page.locator('.wam-cell').count(),64);
    ...
    assert.deepEqual(failures,[]);console.log('PASS: representative layouts, obstacles, decorations, targets, swipe controls, last-move win, next level, locked previews, map spacing, tablet layout and original home-to-map-to-game return flow.');
