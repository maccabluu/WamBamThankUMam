...
const empty=()=>{const g=new Game({...C.level(6),mask:undefined},rng(42));g.board.fill(null);g.bags.clear();return g;};
...
test('all 40 levels start without matches, with a legal move and valid target definitions',()=>{
  for(const config of C.levels)for(let seed=1;seed<=25;seed++){
    const g=new Game(config,rng(seed));assert.equal(g.board.length,g.rows*g.cols);assert.equal(g.scan().cells.length,0);assert.ok(g.findMove());
    assert.equal(g.board.filter(Boolean).length+g.bags.size+g.crates.size+g.holes.size,g.rows*g.cols);
    for(const type of Object.keys(config.targets))assert.ok(config.palette.includes(type)||['handbag','disco','crate','ice'].includes(type));
...
test('levels 6-9 use every supplied icon and four different objective sets',()=>{
  const used=new Set(C.levels.slice(5,9).flatMap(l=>l.palette));assert.equal(used.size,10);
  assert.equal(new Set(C.levels.slice(5,9).map(l=>JSON.stringify(l.targets))).size,4);
...
test('progress preserves existing saves and only advances after a win',()=>{
  ...
  C.complete(s,C.level(9),0,12000);assert.equal(C.progress(s).unlocked,10);assert.equal(C.progress(s).highest,9);assert.equal(C.progress(s).selected,10);
});
test('random legal moves and cascades keep stable boards on all 40 levels',()=>{
  for(const config of C.levels){const g=new Game(config,rng(config.id*99));for(let turn=0;turn<15&&!g.finished;turn++){
    ...
    assert.equal(g.board.filter(Boolean).length+g.bags.size+g.crates.size+g.holes.size,g.rows*g.cols);assert.ok(Object.values(g.targets).every(v=>v>=0));
    if(!g.findMove())g.reshuffle();assert.ok(g.findMove());
  }}
});

test('shaped boards reject holes, cannot match across gaps and refill each section',()=>{
  const g=new Game(C.level(14),rng(4));assert.ok(g.holes.size>0);g.bags.clear();g.crates.clear();g.board.fill(null);
  const hole=[...g.holes][0];assert.equal(g.attempt(hole,hole+1).valid,false);assert.equal(g.useBooster('hammer',hole),null);
  const left=g.piece('lips'),right=g.piece('bow');g.board[g.key(0,0)]=left;g.board[g.key(0,4)]=right;g.fall();
  assert.equal(g.board[g.key(8,0)].id,left.id);assert.equal(g.board[g.key(8,4)].id,right.id);g.holes.forEach(i=>assert.equal(g.board[i],null));
  g.board.fill(null);fill(g,[1,2,4,5],'lips');assert.equal(g.scan().cells.length,0);
});
test('ice stays at its cell, needs the configured hits and counts only once',()=>{
  const g=empty();g.ice.set(1,2);g.targets={ice:1,lips:6};fill(g,[0,1,2]);
  g.clear({cells:[0,1,2,1],spawns:[]});assert.equal(g.ice.get(1),1);assert.equal(g.targets.ice,1);
  g.fall();assert.equal(g.ice.get(1),1);g.clear({cells:[1],spawns:[]});assert.equal(g.ice.size,0);assert.equal(g.targets.ice,0);
});
test('gift boxes take one adjacent hit per wave and never allow gravity through',()=>{
  const g=empty();g.crates.set(7,2);g.targets={crate:1,lips:10};fill(g,[1,6,8,13]);
  g.clear({cells:[1,6,8,13],spawns:[]});assert.equal(g.crates.get(7),1);assert.equal(g.targets.crate,1);
  g.fall();assert.equal(g.board[7],null);g.clear(g.useBooster('hammer',7));assert.equal(g.crates.size,0);assert.equal(g.targets.crate,0);
});
test('a square from Level 10 creates a Flying Kiss and seeks an outstanding target',()=>{
  const g=new Game(C.level(10),rng(7));g.board.fill(null);g.bags.clear();g.crates.clear();g.ice.clear();
  fill(g,[16,17,24,25]);const wave=g.scan([16]);assert.equal(wave.spawns[0].special,'flyer');
  g.clear(wave);g.targets={ice:1};g.ice.set(54,1);g.board[54]=g.piece('bow');const moves=g.moves;
  const activation=g.activate(16);assert.equal(g.moves,moves-1);g.clear(activation);assert.equal(g.targets.ice,0);
});
test('every special pair combination resolves once on a shaped board',()=>{
  for(const a of ['row','column','wrapped','disco','flyer'])for(const b of ['row','column','wrapped','disco','flyer']){
    const g=new Game(C.level(10),rng(12));g.board[16]=g.piece(a==='disco'?'disco':'lips',a);g.board[17]=g.piece(b==='disco'?'disco':'bow',b);
    const move=g.attempt(16,17);assert.ok(move.valid);const cleared=g.clear(move.wave);assert.equal(new Set(cleared.cells).size,cleared.cells.length);g.fall();
    assert.equal(g.board.filter(Boolean).length+g.holes.size+g.bags.size+g.crates.size,g.rows*g.cols);
  }
});
test('decoration currency grants one star per first completion and survives replay',()=>{
  const s=memory();assert.equal(C.renovation(s).available,0);assert.equal(C.decorate(s,0,0),false);
  C.complete(s,C.level(1),10,100);C.complete(s,C.level(2),10,100);assert.equal(C.renovation(s).available,2);
  assert.equal(C.decorate(s,0,0),true);assert.equal(C.renovation(s).available,0);assert.equal(C.decorate(s,0,0),false);
  C.complete(s,C.level(1),15,200);assert.equal(C.renovation(s).available,0);assert.equal(C.decorate(s,1,0),false);
  s.setItem('wambam-unlocked-level',10);s.setItem('wambam-completed-level',9);assert.equal(C.renovation(s).available,7);
  assert.equal(C.decorate(s,1,0),false);C.complete(s,C.level(10),10,200);assert.equal(C.decorate(s,1,0),true);
});
