(function (root, factory) {
  ...
  // Early levels now use the same square-cell shaped boards as the campaign.
  const earlyShapes=['arch','stage','hourglass','wings','window','stage','arch','wings','hourglass'];
  for(let i=0;i<9;i++){
    const old=levels[i],shape=earlyShapes[i],mask=SHAPES[shape];
    const bags=old.bags.length?obstacleCells(mask,old.bags.length,old.id):[];
    levels[i]=Object.freeze({...old,shape,mask,bags});
  }
  const PALETTES = [
  ...
  designs.forEach(([name,shape,paletteId,kind,count,moves],index)=>{
    ...
    levels.push(Object.freeze({id,name,moves,mask,shape,area:Math.min(3,Math.floor((id-1)/10)),difficulty:id%10===0?'Finale':id%5===0?'Hard':'Normal',squareSpecial:'flyer',targets,palette,bags,crates,ice,tip,background:id>=21&&id<=30?'artwork/disco-diner.jpg':'game_background_90.png'}));
  });
  const AREAS=[
