(function () {
  ...
  const css=`
    ...
    .wam-cell img.supplied{mix-blend-mode:multiply}
    .wam-cell[data-hole=true]{visibility:hidden;pointer-events:none}.wam-grid.shaped .wam-cell{background:linear-gradient(#fffaf0,#f8deb1);border:2px solid #bc6c97;border-radius:11px;box-shadow:inset 0 2px #fff,0 2px 3px #602d5844}
    .wam-ice-layer{position:absolute;inset:3px;z-index:4;border:3px solid #4dd4e9;background:linear-gradient(135deg,#c8ffff70 10%,#fff8 12%,#76cfea25 16%,#c7faff88 75%,#fff9 78%,#78d9ee66 83%);border-radius:10px;pointer-events:none;color:#044d69;text-shadow:0 1px white;font-size:15px;font-weight:900;text-align:right;padding:2px}
    .wam-ice-layer[hidden]{display:none}.wam-cell[data-special=flyer]{box-shadow:inset 0 0 0 4px #fc68a6}.wam-cell[data-crate-hits="2"]:after{content:'×2';position:absolute;right:4px;bottom:4px;z-index:5;background:#793683;border:2px solid #ffedb2;border-radius:12px;color:#fff;padding:0 5px;font-size:18px;font-weight:900}
    .wam-board-info{position:absolute;left:114px;top:344px;width:492px;display:flex;justify-content:center;gap:10px;color:#6b2748;font-weight:900;font-size:20px}.wam-board-info span{border:2px solid #b27670;border-radius:20px;padding:7px 13px;background:#fff2d5}
    .wam-shape-tip{position:absolute;left:126px;top:1002px;width:468px;text-align:center;color:#642947;font-weight:800;font-size:20px;line-height:1.35}
    .wam-mini-board{display:grid;gap:3px;width:200px;margin:20px auto}.wam-mini-board span{height:18px;border-radius:3px;background:#ffdaaa}.wam-mini-board span.hole{background:transparent}.wam-mini-board span.blocker{background:#db55a5}.wam-mini-board span.frozen{background:#5de1f1}.wam-cell[data-special=disco] img{width:155%;left:-27.5%;object-fit:cover;clip-path:circle(34%)}
    ...
    .wam-diner .wam-board-title{color:#fff3d3;background:#281126ed;border:2px solid #daa55c;border-radius:14px;padding:5px 10px;height:42px}
    .wam-diner .wam-shape-tip{background:#281126ed;color:#fff3d3;border-radius:12px;padding:7px}
  ...
  class GameView {
    constructor(scene) {
      ...
      if(this.config.id>=21&&this.config.id<=30)this.stage.classList.add('wam-diner');
      ...
      this.board=el('div','wam-grid');this.board.setAttribute('role','group');this.board.setAttribute('aria-label',`Match board, ${this.game.cols} columns and ${this.game.rows} rows`);this.stage.append(this.board);
      this.cellWidth=504/this.game.cols;this.cellHeight=746.88/this.game.rows;
      this.board.style.gridTemplateColumns=`repeat(${this.game.cols},1fr)`;this.board.style.gridTemplateRows=`repeat(${this.game.rows},1fr)`;
      if(this.config.mask) {
        this.cellWidth=this.cellHeight=Math.min(504/this.game.cols,640/this.game.rows);
        const width=this.game.cols*this.cellWidth,height=this.game.rows*this.cellHeight;
        this.board.style.width=width+'px';this.board.style.height=height+'px';this.board.style.left=(360-width/2)+'px';this.board.style.top=(430+(620-height)/2)+'px';this.board.classList.add('shaped');
        const info=el('div','wam-board-info');info.append(el('span','',this.config.difficulty.toUpperCase()),el('span','',C.AREAS[this.config.area].name));this.stage.append(info);
        const tip=el('div','wam-shape-tip',this.config.squareSpecial==='flyer'?'4 in a square = Flying Kiss · 5 in a line = Disco':'4 in a square or 5 in a line = Disco');tip.style.top=Math.min(1030,430+(620+height)/2+20)+'px';this.stage.append(tip);
      }
      this.cells=Array.from({length:this.game.rows*this.game.cols},(_,i)=>this.createCell(i));
      ...
      this.toast(this.config.name.toUpperCase(),1600);
    }
    ...
    createCell(i) {
      ...
      const img=el('img');img.draggable=false;const mark=el('span','wam-special-mark'),iceLayer=el('span','wam-ice-layer');iceLayer.hidden=true;b.append(img,mark,iceLayer);
      b.addEventListener('pointerdown',e=>{
        if(!this.game.valid(i)||e.button!==0||this.busy||this.paused||this.ended||this.drag)return;
      ...
      b.addEventListener('pointerup',e=>{
        ...
        const {r,c}=this.game.coords(i),horizontal=Math.abs(dx)>Math.abs(dy),nr=r+(horizontal?0:Math.sign(dy)),nc=c+(horizontal?Math.sign(dx):0);
        if(nr>=0&&nr<this.game.rows&&nc>=0&&nc<this.game.cols)void this.swap(i,this.game.key(nr,nc));
      });
      ...
      this.board.append(b);return {b,img,mark,iceLayer};
    }
    ...
    render(){
      ...
      this.cells.forEach(({b,img,mark,iceLayer},i)=>{
        const p=this.game.board[i],bag=this.game.bags.get(i),crate=this.game.crates.get(i),ice=this.game.ice.get(i),type=bag?'handbag':crate?'crate':['disco','flyer'].includes(p?.special)?p.special:p?.type,def=C.PIECES[type];
        b.dataset.hole=String(this.game.holes.has(i));b.disabled=this.game.holes.has(i);b.dataset.crateHits=String(crate||0);
        iceLayer.hidden=!ice;iceLayer.textContent=ice>1?'2':'';
        ...
        const special=bag||crate?'':p?.special,labels={row:'↔',column:'↕',wrapped:'✦',flyer:'♥'};mark.textContent=labels[special]||'';mark.hidden=!labels[special];mark.style.display=labels[special]?'flex':'none';
        const {r,c}=this.game.coords(i);b.setAttribute('aria-label',`${bag?'Handbag, '+bag+' hits left':crate?'Gift box, '+crate+' hits left':def?.label||'Empty'}${ice?', ice '+ice+' hits left':''}${special?' '+special:''}, row ${r+1}, column ${c+1}`);
      });
      this.board.setAttribute('aria-busy',String(this.busy));
    }
    async tap(i){
      if(!this.game.valid(i)||this.busy||this.paused||this.ended||this.disposed)return;audio();this.clearHint();
      ...
      if(this.game.blocked(i)){this.selected=null;this.toast('Match beside the obstacle or use a special');this.render();return;}
      if(!this.booster&&this.game.board[i]?.special&&this.selected===null){const wave=this.game.activate(i);if(wave){this.busy=true;this.render();await this.resolve(wave);}return;}
    ...
    async swap(a,b){
      ...
      if(!this.game.valid(a)||!this.game.valid(b))return;
      if(this.game.blocked(a)||this.game.blocked(b)){this.toast('Match beside the obstacle or use a special');return;}
      this.busy=true;this.selected=null;this.clearHint();this.render();
      const x=this.game.coords(a),y=this.game.coords(b),dx=(y.c-x.c)*this.cellWidth,dy=(y.r-x.r)*this.cellHeight;
    ...
    async resolve(first){
      ...
      try {
        while(wave?.cells.length){
          ...
          if(wave.label)this.toast(wave.label);else if(combo>1)this.toast(`COMBO ×${combo}`);else if(wave.spawns?.length)this.toast(wave.spawns.some(s=>s.special==='disco')?'DISCO CREATED!':wave.spawns.some(s=>s.special==='flyer')?'FLYING KISS!':'SPECIAL CREATED!');
          ...
          const drops=falls.map(f=>this.animate(this.cells[f.at].img,[{transform:`translateY(${-f.distance*this.cellHeight}px)`,opacity:.5},{transform:'translateY(0)',opacity:1}],{duration:Math.min(380,200+f.distance*22),easing:'cubic-bezier(.3,.6,.4,1)',fill:'both'}));
    ...
    pause(force=false){
      ...
      if(this.game.won){
        ...
        const modal=dialog(this.stage,`Level ${this.config.id} complete!`,this.config.id===C.TOTAL?`All ${C.TOTAL} levels complete. Decorate your areas or replay for more stars.`:`${this.config.name} complete. Level ${this.config.id+1} is ready.`);
        modal.card.prepend(el('div','wam-eyebrow','WAM BAM · THANK U MAM'));modal.card.append(el('div','wam-stars','★'.repeat(stars)+'☆'.repeat(3-stars)),el('p','',`${this.game.score.toLocaleString()} points · ${this.game.moves} moves left`),el('p','',firstWin?'+250 coins · +1 decoration star':'+250 coins'));
