(function(){
  ...
  class MapView {
    constructor(scene){
      ...
      this.view=U.createHost('wambam-level-map','artwork/road-map.jpg');this.stage=this.view.stage;this.installStyle();
    ...
    installStyle(){
      if(document.getElementById('wam95-map-style'))return;const s=el('style');s.id='wam95-map-style';s.textContent=`
        ...
        .wam-map-world{position:relative;width:100%;height:1280px;background:#28495e url('artwork/road-map.jpg') center top / 100% 100% no-repeat}
        .wam-route{position:absolute;inset:0;width:100%;height:100%;pointer-events:none}.wam-area-title{position:absolute;top:14px;left:90px;right:90px;text-align:center;color:#ffdea2;background:#23132adb;border:2px solid #d4a655;border-radius:18px;padding:10px}.wam-area-title h2{font-size:28px;margin:4px}.wam-area-title p{font-size:18px;margin:4px;color:#ffe9cd}
        .wam-map-stop{position:absolute;width:90px;transform:translate(-50%,-50%);text-align:center;color:#fff}
        ...
        .wam-node-lock{position:absolute;right:-6px;bottom:-3px;font-size:25px}.wam-node-stars{font-weight:900;font-size:17px;color:#ffde78;letter-spacing:1px;text-shadow:0 2px 4px #000;margin-top:3px}
        ...
        .wam-node-you{position:absolute;top:-25px;left:-4px;right:-4px;border-radius:8px;background:#fff0a6;color:#731544;padding:5px;font-size:13px;font-weight:900}
    ...
    showArea(index){
      ...
      configs.forEach((config,i)=>{
        ...
        const stars=this.progress.stars[config.id-1];stop.append(el('div','wam-node-stars',stars?'★'.repeat(stars)+'☆'.repeat(3-stars):completed?'✓':'☆ ☆ ☆'));
      ...
      this.scroller.scrollTop=1280;this.selected=area.from;
    }
