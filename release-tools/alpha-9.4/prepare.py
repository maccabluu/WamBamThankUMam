#!/usr/bin/env python3
"""Apply the 9.4 web game to the verified 9.3 APK's extracted www folder.
...
data.write_text(project.replace('"version":"9.3.0"', '"version":"9.4.0"', 1))
...
text, count = re.subn(r'const CURRENT_VERSION = "[^"]+";', 'const CURRENT_VERSION = "9.4.0";', text, count=1)
...
text = text.replace('Swap two icons to match 3 or more. Make a 2 × 2 square for a Disco. Swap a Disco with an icon to turn all matching icons into Discos, clear targets and earn coins.',
                    'Swap neighbours to match 3. Match 4 for a line blast. A T or L creates an area blast. Match 5 or a square for a Disco. Combine specials, collect targets and earn stars across 9 levels.')
...
print(json.dumps({'version': '9.4.0', 'levels': 9, 'new_icons': len(list((root/'icons').glob('*.png'))), 'www': str(root)}))
